...
#define ESAMPLE 61035 // 0.5 seconds in
...
void usage()
{
  fprintf (stdout,
	   ...
	   " -n filter width [default 0]\n"
	   " -b number of bins [default 16]\n"
	   " -d DM [default 56.78]\n"
	   " -f locations of dsa1-5 raw data files [default dsa1.out...]\n"
	   " -g location of output file (default test.fits)\n"
	   " -j location of bandpass.dat file (default in IMAGING dir)\n"
	   " -h        print usage\n");
}
...
int main (int argc, char *argv[]) {
  ...
  char bploc[100];
  sprintf(bploc,"/home/user/IMAGING/bandpass.dat");
  
  for (int i=1;i<argc;i++) {

    if (strcmp(argv[i],"-j")==0)
      strcpy(bploc,argv[i+1]);
    
    if (strcmp(argv[i],"-m")==0) 
      mjd = (double)(atof(argv[i+1]));

    if (strcmp(argv[i],"-n")==0)
      samples_per_bin = (atoi(argv[i+1]));
  ...
  long long start_sample = (long long)(ESAMPLE-(number_of_bins/2)*samples_per_bin);
  printf("MJD: %lf\n",mjd);
  printf("start sample: %llu\n",start_sample);
  printf("samples per bin: %d\n",samples_per_bin);
  ...
  sprintf(antennas,"1-4-5-8-6-9-2-10-3-7");
  fits_write_key(fptr, TSTRING, "ANTENNAS", &antennas[0], "Antennas", &status);
  free(antennas);
  fits_close_file(fptr, &status);

  // read in bandpass scaling factors
  float bandpass[2][10][2048];
  FILE *fbp;
  fbp = fopen(bploc,"r");
  for (int i=0;i<2;i++) { // pol
    for (int j=0;j<10;j++) { // antenna
      for (int k=0;k<2048;k++)  // chan
	fscanf(fbp,"%f\n",&bandpass[i][j][k]);
    }
  }
  fclose(fbp);
  ...
  float tofile[2][220], wtofile[220];
  ...
  for (int tidx=0;tidx<number_of_bins;tidx++) {
	...
	for (int i=0;i<220;i++) wtofile[i] = 0.;
	...
	for (int bidx=0;bidx<samples_per_bin;bidx++) {
	  ...
	  for (int i=0;i<10;i++) {
	    ...
	    wtofile[ci] += sqrt(bandpass[0][i][dmi])*((float)(((int)(d[0][0][i])*1.))); // AR
	    ci++;
	    wtofile[ci] += sqrt(bandpass[0][i][dmi])*((float)(((int)(d[0][1][i])*1.))); // AI
	    ci++;
	    wtofile[ci] += sqrt(bandpass[1][i][dmi])*((float)(((int)(d[1][0][i])*1.))); // BR
	    ci++;
	    wtofile[ci] += sqrt(bandpass[1][i][dmi])*((float)(((int)(d[1][1][i])*1.))); // BI
	    ci++;
	...
	wtofile[i] = tofile[1][i];
      fits_write_col(fptr, TFLOAT, 1, rownum, 1, 220, wtofile, &status);
      rownum+=220;
      for (int i=0;i<220;i++)
